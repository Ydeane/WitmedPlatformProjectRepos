<template>
  <div>
    <div class="login-box">
      ...
      <form>
        <div class="user-box">
          <input type="text" name="" required="" v-model="cellphone">
          <label>用户名</label>
        </div>
        <div class="user-box">
  ...
  methods: {
    loginAdmin: function() {
      this.$axios.post("http://localhost:11111/system/admin/admin/login",{
        ...
        let token=  localStorage.getItem(resp.headers['authorization']);
        console.log(token);
        //获得响应的结果编码
        var ResponseCode = resp.data.code;
        //判断编码是否为20000
        if (ResponseCode == 20000){
          //响应成功，切换到首页面
          this.$router.push("/index");
          localStorage.setItem("au","Bearer " +  resp.headers['authorization'])
        }
      })
    }
  }
